...
def callbackF(Xi):
    ...
    indexx=indexx+1
    global solutionx
    solutionx = np.append(solutionx, [temp], axis=0)
...
cons=({'type': 'eq',
       'fun': cons1})
optimize_para=optimize.minimize(differnce,W_j,method='SLSQP',bounds=bnds,constraints=cons,callback=callbackF,options={'maxiter':100,'disp': False})
...
df_so=np.concatenate((df_so,solutionx[:,7].T[:, None]),axis=1)
...
ax.plot3D(X_train_pca[:,0], X_train_pca[:,1], solutionx[:,7], 'red')
ax.scatter3D(X_train_pca[:,0], X_train_pca[:,1], solutionx[:,7], c=solutionx[:,7], cmap='Greens');
ax.set_xlabel('PC 1')
ax.set_ylabel('PC 2')
ax.set_zlabel('MDD')
...
Z = np.outer(solutionx[:,7].T, solutionx[:,7])
X, Y = np.meshgrid(X_train_pca[:,0], X_train_pca[:,1])
...
if cons_trant==0:
    print('no con')
    plt.savefig('plot_fminSL.png')
    df_so.to_csv('fminSL_dfsolution.csv', index=False)
else:
    print('con')
    plt.savefig('plot_fminSL_con.png')
    df_so.to_csv('fminSL_dfsolution_con.csv', index=False)